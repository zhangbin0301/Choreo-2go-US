...
const NEZHA_SERVER = process.env.NEZHA_SERVER || 'nazhe.841013.xyz';     
const NEZHA_PORT = process.env.NEZHA_PORT || '443';                     // 哪吒端口为{443,8443,2096,2087,2083,2053}其中之一开启tls
const NEZHA_KEY = process.env.NEZHA_KEY || 'rIJRezDvaD0J55RIEm1';   // 没用哪吒，把这个参数空着
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'choreo-2go-us.841013.xyz';     // 建议使用token，argo端口http 8080，cf后台设置需对应,使用json需上传json和yml文件至files目录
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiMzJkNmUxMmQxZDAzOGRjZTYxODZhMzkwYmZjN2EyY2UiLCJ0IjoiNWY2YjJiNmYtYWEzMi00YTcyLWJkMjEtNDgyOGYwMjk4Mjg3IiwicyI6Ik1XVTFORFpoTm1RdE1HSTROeTAwTXpRM0xXSTFNVGN0TkRoaE5XWTFZalpoTnpjMiJ9';
const CFIP = process.env.CFIP || 'ip.sb';
const NAME = process.env.NAME || 'Choreo.dev-MSN';
...
app.get('/sub', (req, res) => {
  ...
  const vlessURL = `vless://${UUID}@${CFIP}:443?encryption=none&security=tls&sni=${ARGO_DOMAIN}&type=ws&host=${ARGO_DOMAIN}&path=%2Fvless-argo%3Fed%3D2048#${ISP}-${NAME}`;
